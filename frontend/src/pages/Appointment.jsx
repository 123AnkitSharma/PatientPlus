...
const Appointment = () => {
    ...
    useEffect(() => {
        const firstAvailable = docSlots.findIndex(slots => slots.length > 0)
        if (firstAvailable !== -1) {
            ...
            <div className='sm:ml-72 sm:pl-4 mt-8 font-medium text-[#565656]'>
                ...
                <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
                    {docSlots.length && docSlots.map((item, index) => (
                        item.length > 0 && <div onClick={() => setSlotIndex(index)} key={index} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-primary text-white' : 'border border-[#DDDDDD]'}`}>
